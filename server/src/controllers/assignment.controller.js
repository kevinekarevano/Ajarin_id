...
export const submitAssignment = async (req, res) => {
  try {
    ...
    console.log("Assignment submission:", { assignmentId, studentId, submissionType, hasFile: !!file });

    // Validate assignment exists and is an assignment type
    ...
    if (submissionType === "text") {
      ...
      if (!file) {
        return res.status(400).json({
          success: false,
          message: "File is required for file submissions",
        });
      }
      ...
      if (file.size > maxFileSize * 1024 * 1024) {
        return res.status(400).json({
          success: false,
          message: `File size cannot exceed ${maxFileSize}MB`,
        });
      }
    ...
    if (existingSubmission) {
      ...
      submission = new AssignmentSubmission({
        ...
        due_date: assignment.assignment_data?.due_date,
        status: "submitted",
      });
